...
def _get_detections(generator, model, word_seen, word_unseen, top, score_threshold=0.05, max_detections=100, save_path=None):
    ...
    for i in range(generator.size()):
        ...
        T = top
        #create an ndarray with shape scores.shape
...
def evaluate(
    ...
    """
    ## Shafin
    if wordvector == 'word_w2v':
        word = np.loadtxt('ZJZSD/word_w2v.txt', dtype='float32', delimiter=',')
        voc = np.loadtxt('ZJZSD/vocabulary_w2v.txt', dtype='float32', delimiter=',')
    elif wordvector == 'word_glo':
        word = np.loadtxt('ZJZSD/word_glo.txt', dtype='float32', delimiter=',')
        voc = np.loadtxt('ZJZSD/vocabulary_glo.txt', dtype='float32', delimiter=',')

    wei = model.layers[210].get_weights()[10]
    word = np.tanh(np.dot(np.dot(voc, wei), word))

    word_seen = word[:, :seenclass]
    word_unseen = word[:, seenclass:]

    # gather all detections and annotations
    all_detections     = _get_detections(generator, model, word_seen, word_unseen, top, score_threshold=score_threshold, max_detections=max_detections, save_path=save_path)
