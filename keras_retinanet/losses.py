...
def polar(alpha=0.25, gamma=2.0):

    def _polar(y_true, y_pred):
        labels         = y_true
        classification = y_pred
        ...
        loc = backend.where(keras.backend.equal(labels, 1), labels, keras.backend.zeros_like(labels))
        pos = loc * y_pred
        maxpos = keras.backend.max(pos, axis=2, keepdims=True)
        maxposrep = keras.backend.repeat_elements(maxpos, int(y_pred.shape[2]), 2)
        maxloc = keras.backend.max(loc, axis=2, keepdims=True)
        maxlocrep = keras.backend.repeat_elements(maxloc, int(y_pred.shape[2]), 2)
        diff1 = maxlocrep*y_pred - maxposrep

        # to manage all zero gt mean background
        state = keras.backend.max(labels, axis=2, keepdims=True)
        loc = backend.where(keras.backend.equal(state, 0), keras.backend.ones_like(state), keras.backend.zeros_like(state))
        maxlocrep = keras.backend.repeat_elements(loc, int(y_pred.shape[2]), 2)
        diff2 = maxlocrep * y_pred

        diff = diff1 + diff2

        # Max margin softplus
        # logediff = keras.backend.log(1+keras.backend.exp(5*diff))

        # Max margin sigmoid
        # ex = keras.backend.exp(5*(diff - 0.5))
        # logediff = ex / (1 + ex)
        beta = 0.5
        ex = keras.backend.exp(-beta*(diff))
        logediff = 1 / (1 + ex)

        cls_loss = logediff * cls_loss
